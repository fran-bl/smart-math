...
ALGORITHM = (settings.ALGORITHM or "HS256").strip()
...
async def create_user(db: db_dependency, create_user_request: CreateUserRequest):
    ...
    create_user_model = User(
        ...
        or None,  # TODO: ako hashiramo lozinku u bazi -> password=pwd_context.hash(create_user_request.password),
...
@router.post(
    ...
    summary="login for students and teachers separated",
)
async def login_for_access_token(form_data: MyLoginForm, db: db_dependency):
...
def authenticate_user(username: str, password: str, db):
    ...
    ):  # pwd_context.verify(password, user.hashed_password): #provjeri je li dobar password
        return False
    return user
